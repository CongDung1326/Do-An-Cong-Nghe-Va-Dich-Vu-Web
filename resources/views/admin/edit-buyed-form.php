...
<?php if ($check_type['type'] == "lol"): ?>
    ...
    WHERE (a.id = l.account_id AND l.rank_lol_id = i.id) AND a.is_sold = 'T' AND a.type = 'lol' AND a.id = $id;";
...
<?php else: ?>
    <?php
    $query = "SELECT a.id, a.username, a.password, s.title FROM account a, store_account_children s WHERE a.store_account_children_id = s.id AND a.is_sold = 'T' AND a.id=$id";
    $item = $call_db->get_row($query);
