...
if (input_get(hash_encode("limit-order")) && input_get(hash_encode("limit"))) {
    ...
    $limit = $limit_max == 1 ? 0 : $limit_max * ($limit_order - 1);

    $query = "SELECT store_account_children_id, account_lol_id FROM notification_buy ORDER BY time DESC LIMIT $limit,$limit_max";
